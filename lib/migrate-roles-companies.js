...
const { MEDSUPPLY_ORGANIZATION_NAME, ensureMedSupplyCompany } = require('./medsupply-organization');
...
async function ensureRolesAndCompanies() {
  ...
  const medsupplyCompanyId = await ensureMedSupplyCompany(pool);
  await pool.query(
    `UPDATE users SET company_id = ?, company = ? WHERE role IN ('SYSTEM_ADMIN', 'ADMIN')`,
    [medsupplyCompanyId, MEDSUPPLY_ORGANIZATION_NAME]
  );
