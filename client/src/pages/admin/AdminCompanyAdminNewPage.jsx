...
export default function AdminCompanyAdminNewPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const result = await fetchAdmin(`/companies/${id}/admins-new${searchParams.toString() ? `?${searchParams.toString()}` : ''}`);
      if (cancelled) return;
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      <PageHeader title="Add client admin" subtitle={`Organization: ${c.name}`} />
      ...
      <form
        method="post"
        action={`/admin/companies/${id}/admins`}
