...
const useS3 = (process.env.STORAGE_DRIVER || 'local').toLowerCase() === 's3';
const multerDestination = useS3 ? path.join(uploadDir, 'tmp') : uploadDir;
if (useS3 && !fs.existsSync(multerDestination)) {
  fs.mkdirSync(multerDestination, { recursive: true });
}
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => cb(null, multerDestination),
...
module.exports = { upload, ALLOWED_EXTENSIONS, MAX_SIZE, uploadDir, multerDestination, useS3 };
